# Concepts

This folder defines the canonical vocabulary for Remram Cortex.

These documents are intentionally short and conceptual. Use them to align terminology before editing the architecture or implementation.

If you are new to the repository, read [Overview](../overview/overview.md) first.
Read [Glossary](../glossary.md) before this folder if you need the locked short-form definitions.

## Core Active Concepts

These are directly part of the active layered architecture:

- [Knowledge Authority](knowledge-authority.md)
- [Memory Versus Context](memory-vs-context.md)
- [Knowledge Object](knowledge-object.md)
- [Notion](notion.md)
- [Evidence Package](evidence-package.md)
- [High-Signal Mamba Stream](high-signal-mamba-stream.md)
- [Reflection](reflection.md)
- [Intuition](intuition.md)
- [Dream Cycle](dream-cycle.md)
- [Artifact Intake](artifact-intake.md)
- [Artifact Provider](artifact-provider.md)
- [Artifact Promotion](artifact-promotion.md)
- [Bootstrap Ingestion](bootstrap-ingestion.md)
- [Bounded Retrieval](bounded-retrieval.md)

## Supporting Concepts

These remain compatible with the active design but are narrower or more implementation-shaped:

- [Governance Fields](governance-fields.md)
- [Conversation Layer](conversation-layer.md)

## Retained Retrieval Concepts

These are retained from earlier Cortex thinking and may still be useful later, but they are not locked phase-1 implementation commitments:

- [Typed Signals](typed-signals.md)
- [Semantic Signature](semantic-signature.md)
- [Dimension (Legacy Term)](dimension.md)

## Suggested Reading Order

1. [Knowledge Authority](knowledge-authority.md)
2. [Memory Versus Context](memory-vs-context.md)
3. [Knowledge Object](knowledge-object.md)
4. [Notion](notion.md)
5. [Evidence Package](evidence-package.md)
6. [High-Signal Mamba Stream](high-signal-mamba-stream.md)
7. [Reflection](reflection.md)
8. [Intuition](intuition.md)
9. [Dream Cycle](dream-cycle.md)
10. [Artifact Intake](artifact-intake.md)
11. [Artifact Provider](artifact-provider.md)
12. [Artifact Promotion](artifact-promotion.md)
13. [Bootstrap Ingestion](bootstrap-ingestion.md)
14. [Bounded Retrieval](bounded-retrieval.md)
15. [Governance Fields](governance-fields.md)
16. [Conversation Layer](conversation-layer.md)
17. [Typed Signals](typed-signals.md)
18. [Semantic Signature](semantic-signature.md)
19. [Dimension (Legacy Term)](dimension.md)

Go next:

- [Overview](../overview/overview.md) if you need the high-level role and repository boundary
- [Layered Memory Architecture](../design/layered-memory-architecture.md) for the active system design
